...
To ensure the integrity and standardization of the Olympic host dataset, we utilize OpenRefine, a powerful tool for cleaning messy data. This step transforms the raw Wikipedia scrape into a structured, analysis-ready format by applying a predefined cleaning recipe.

Step 1: Project Creation

- Launch OpenRefine in your browser (typically at http://localhost:3333).

- Click on "Create Project" and select "Get data from This Computer".

- Upload the extracted-tables.csv file generated by the scraper.

- Review the preview and ensure the character encoding is set to UTF-8. Click "Create Project".

Step 2: Applying the Cleaning Schema (history.json)

- Rather than performing manual cleaning steps, we apply a saved operation history to ensure reproducibility.

- In the top-left panel, click on the "Undo / Redo" tab.

- Click the "Apply..." button at the bottom of the panel.

- Open your `data/history.json` file in a text editor, copy the entire JSON array, and paste it into the text area.

- Click "Perform Operations".

Step 3: Exporting the Cleaned Dataset

- Click the "Export" button in the top-right corner.

- Select "Comma-separated value (.csv)".

- Save the file as `olympic_hosts_clean.csv` in your `data/processed/` directory.
